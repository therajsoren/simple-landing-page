...
const Navbar = () => {
  ...
  return (
    <nav className="border-b-2">
         {/* <div className="max-w-7xl mx-auto flex justify-between items-center py-8"></div> */}
        <div className=" max-w-7xl mx-auto flex justify-between items-center py-8">
        <div className="pl-2">
            <a href="#">
                <img src={logo} height={15} width={150}/>
            </a>
        </div>
        ...
        <div className={`${isOpen ? "block": "hidden"} md:hidden absolute bg-neutral-50 w-full py-5 px-4 mt-2 border-b-4`}>
            {LINKS.map((link, index) => (
                <a key={index} href={link.link} className="text-black uppercase text-lg font-medium block py-2 -tracking-wide">
                    {link.name}
                </a>
            ))}
        </div>
